<!doctype html>
<html>
<head>
<meta charset="UTF-8">
<title>All Winter 2017 Courses | Winter Session at UMBC</title>

<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
<meta name="description" content="Use your smart phone, mobile device, or computer to register through myUMBC." />

<!-- Twitter Card data -->
<meta name="twitter:card" value="summary">
<meta name="twitter:site" content="@UMBCWinter">

<!-- Open Graph data -->
<meta property="og:title" content="Application Process for Graduate Students | Winter Session at UMBC" />
<meta property="og:type" content="article" />
<meta property="og:url" content="http://www.umbc.edu/winter/applygraduate" />
<meta property="og:image" content="http://www.umbc.edu/winter/images/1.jpg" />
<meta property="og:description" content="Use your smart phone, mobile device, or computer to register through myUMBC." />


<!-- CSS -->
<link href='http://fonts.googleapis.com/css?family=Roboto:300,400,500,600,700' rel='stylesheet' type='text/css'>
<link href="css/font-awesome.min.css" rel="stylesheet">
<link href="css/styles.css" rel="stylesheet">
<link href="css/magnetic-popup.css" rel="stylesheet">
<link href="css/accordion.css" rel="stylesheet">

<!-- Javascript -->
<script src="js/modernizr.custom.js"></script>

<!-- Favicons -->
<link rel="shortcut icon" href="images/favicon.ico">
<link rel="apple-touch-icon" href="images/apple-touch-icon.png">
<link rel="apple-touch-icon" sizes="72x72" href="images/apple-touch-icon-72x72.png">
<link rel="apple-touch-icon" sizes="114x114" href="images/apple-touch-icon-114x114.png">
</head>

<body>

<div id="skip-content">
 <a href="#main-content">Skip to Main Content</a>
</div>

<div id="container-header-background">
 <!-- UMBC header -->
   <?php include("includes/umbc-header.php"); ?>
 <!-- END umbc header -->
 <div id="site-menu-background">
  <!-- gold toolbar -->
   <?php include("includes/gold-nav.php"); ?>
  <!-- END gold toolbar --> 
 </div>
</div>
<!-- BEGIN MAIN -->
<div class="container column" id="main">  <div  id="winter-heading">
    <a href="index">
    <h1 class="remove-bottom">
    <img src="images/winter-heading.png" style="width:100%">
    </h1>
    </a>
  </div>


 <div class="sixteen columns">

  <!-- Page Title -->
        <br>
<h2>Winter 2017 Courses</h2>
    <!-- END Page Title -->       

      
     <!-- Start Content --> 
<p>Click on the "More Details" links to provide additional information related to your current search<span class="hide-stem"> (which is shown in the "Search" box)</span>.</p>
<p>You can switch your current search criteria by clicking on the links to the right of any of the following options: Format &amp; Location, Dates &amp; Times, GEPs, or Prerequisites.</p>

<div class="stem">
<h4>The 2016 list of Summer STEM courses includes previous course syllabi and 2016 course start dates.</h4>

<p>The <strong>course syllabi</strong> are based on prior semesters and  should be used as a planning tool. Current course syllabi are distributed by  course instructions at the beginning of each course. <em>Please note that there  may not be a course syllabus for every listed course.</em><br>
  </p>
<p>The <strong>Session Start Dates</strong> provide a quick reference to  assist in creating a course schedule. The specific meeting schedule and  locations are available via the course catalog or course schedule.<br>
  </p>
<p>Many summer session courses have <strong>limited seats</strong>.  Enroll <strong>early</strong> to secure your seat in the class. To check how many seats  are available in a specific course, click on Course Title. This takes you to  the schedule of classes. Click on &quot;View Section&quot; to open the  &quot;Class Details&quot; screen.&nbsp;See &quot;Class Availability&quot; to  view the number of open seats, as well as the wait list for closed courses.</p>
</div>


<p style="font-size:125%; line-height:125%" class="searchbox">
<strong>Search:</strong> <input type="text" id="search" placeholder="Type to search" height="30px">
</p>

<p style="font-size:115%; line-height:125%">

<span class="location">
<strong>Format &amp; Location:</strong>
<a href="javascript:void(0);" class="category">Hybrid</a>
<a href="javascript:void(0);" class="category">Online</a>
<a href="javascript:void(0);" class="category">Shady Grove</a>
<a href="javascript:void(0);" class="category last">Main Campus</a>
</span>

<span class="date-times">
<br>
<strong>Dates &amp; Times: </strong>
<a href="javascript:void(0);" class="category">Morning</a> 
<a href="javascript:void(0);" class="category">Afternoon</a> 
<a href="javascript:void(0);" class="category">Evening</a> 
<!-- Remove for the sessions to go away -->
<a href="javascript:void(0);" class="category">Session 1</a> 
<a href="javascript:void(0);" class="category">Session 2</a> 
<a href="javascript:void(0);" class="category last">12 Week Course</a>
<br>
</span>


<span class="geps">
<strong>GEPs:</strong> 
<a href="javascript:void(0);" class="category">Arts and Humanities (AH)</a> 
<a href="javascript:void(0);" class="category">Culture (C)</a> 
<a href="javascript:void(0);" class="category">Language (L)</a> 
<a href="javascript:void(0);" class="category">Math (M)</a>
<a href="javascript:void(0);" class="category">Science (S)</a> 
<a href="javascript:void(0);" class="category">Social Science (SS)</a> 
<a href="javascript:void(0);" class="category last">Writing Intensive (WI)</a><br>
</span>



<span class="stem">
<strong>Subjects:</strong> 
<a href="javascript:void(0);" class="category-stem" name="*BIOL">Biology</a> 
<a href="javascript:void(0);" class="category-stem" name="*CHEM">Chemistry</a> 
<a href="javascript:void(0);" class="category-stem" name="*ENCH">Chemical Engineering</a> 
<a href="javascript:void(0);" class="category-stem" name="*EHS">Emergency Health Services</a>
<a href="javascript:void(0);" class="category-stem" name="*ENME">Mechanical Engineering</a>
<a href="javascript:void(0);" class="category-stem" name="*GES">Geography and Environmental Systems</a> 
<a href="javascript:void(0);" class="category-stem" name="*IS">Information Systems</a>
<a href="javascript:void(0);" class="category-stem" name="*MATH">Mathematics</a> 
<a href="javascript:void(0);" class="category-stem" name="*PHYS">Physics</a>
<a href="javascript:void(0);" class="category-stem" name="*SCI">Science</a>
<a href="javascript:void(0);" class="category-stem last" name="*STAT">Statistics</a><br> 
</span>



<span class="prereq1">
<strong>Prerequisites:</strong> 
<a href="javascript:void(0);" class="category">Has Prerequisite</a> 
<a href="javascript:void(0);" class="category last">No Prerequisite</a><br><br>
</span>



<strong>More Details: </strong><a href="javascript:void(0);" class="instructors details">Show Instructors</a> 
<a href="javascript:void(0);" class="instructors no-instructors details" style="display:none">Hide Instructors</a> |

<a href="javascript:void(0);" class="time details">Show Times</a> 
<a href="javascript:void(0);" class="time no-time details" style="display:none">Hide Times</a> |

<a href="javascript:void(0);" class="sessions details">Show Sessions</a> 
<a href="javascript:void(0);" class="sessions no-sessions details" style="display:none">Hide Sessions</a> |

<a href="javascript:void(0);" class="prereq details">Show Prerequisites</a> 
<a href="javascript:void(0);" class="prereq no-prereq details" style="display:none">Hide Prerequisites</a> |

<a href="javascript:void(0);" class="gep details">Show GEPs</a> 
<a href="javascript:void(0);" class="gep no-gep details" style="display:none">Hide GEPs</a> |

<a href="javascript:void(0);" class="campus details">Show Campus</a> 
<a href="javascript:void(0);" class="campus no-campus details" style="display:none">Hide Campus</a> |

<a href="javascript:void(0);" class="format details">Show Delivery Mode</a> 
<a href="javascript:void(0);" class="format no-format details" style="display:none">Hide Delivery Mode</a> |

<a href="javascript:void(0);" class="days details">Show Days</a> 
<a href="javascript:void(0);" class="days no-days details" style="display:none">Hide Days</a>

</p>

<?php include("../summer/includes/courses.php"); ?>

  </div>
<br clear="all"><br>
  <footer id="site-footer" class="clearfix">
    <?php include("includes/footer.php"); ?>
    <?php include("includes/tracking.php"); ?>
    <?php include("includes/syllabi-list.php"); ?>
  </footer>
</div>
<!-- container --> 

<script src="//code.jquery.com/jquery-1.12.0.js"></script> 
<script src="//code.jquery.com/jquery-migrate-1.3.0.js"></script>
<script src="//code.jquery.com/ui/1.11.4/jquery-ui.js"></script>
<script src="js/scripts.js"></script> 
<script src="js/jquery.magnific-popup.js"></script>
<script src="js/popup.js"></script>  
  
<script>
/* Assign class & ID to table*/
$('table').addClass( "courses" );
$('table').attr('id', 'table');

var $rows = $('#table tr:not(:first)');
$('#search').keyup(function() {
    var val = $.trim($(this).val()).replace(/ +/g, ' ').toLowerCase();
	ga('send', 'event',  'search', 'search', val);

    $rows.show().filter(function() {
        var text = $(this).text().replace(/\s+/g, ' ').toLowerCase();
        return !~text.indexOf(val);
    }).hide();
});

$(function(){
    $('.category').live('click', function() {
        $("#search").val($(this).html());
		$("#search").trigger(e);				
    });
});

$(function(){
    $('.category-stem').live('click', function() {
        $("#search").val($(this).attr('name'));
		$("#search").trigger(e);				
    });
});

var e = jQuery.Event("keyup");
e.which = 13; // # Some key code value


$('td:nth-child(3)').hide();
$('td:nth-child(5)').hide();
$('td:nth-child(6)').hide();
$('td:nth-child(7)').hide();
$('td:nth-child(8)').hide();
$('td:nth-child(9)').hide();
$('td:nth-child(10)').hide();
$('td:nth-child(11)').hide();
$('td:nth-child(12)').hide();
$('td:nth-child(13)').hide();
$('td:nth-child(14)').hide();
$('td:nth-child(15)').hide();
$('td:nth-child(16)').hide();
$('td:nth-child(17)').hide();
$('td:nth-child(18)').hide();
$('td:nth-child(19)').hide();
$('td:nth-child(20)').hide();
$('td:nth-child(21)').hide();
$('td:nth-child(22)').hide();
$('td:nth-child(23)').hide();
$('.stem').hide();

$( '.instructors' ).live( 'click', function() {
  $('td:nth-child(16)').show();
  $('a.instructors').toggle();
});
$( '.no-instructors' ).live( 'click', function() {
  $('td:nth-child(16)').hide();
  
});
$( '.time' ).live( 'click', function() {
  $('td:nth-child(13)').show();
  $('td:nth-child(14)').show();
  $('a.time').toggle();
});
$( '.no-time' ).live( 'click', function() {
  $('td:nth-child(13)').hide();
  $('td:nth-child(14)').hide();
});
$( '.sessions' ).live( 'click', function() {
  $('td:nth-child(5)').show();
  $('td:nth-child(6)').show();
  $('a.sessions').toggle();
});
$( '.no-sessions' ).live( 'click', function() {
  $('td:nth-child(5)').hide();
  $('td:nth-child(6)').hide();
});
$( '.gep' ).live( 'click', function() {
  $('td:nth-child(7)').show();
  $('td:nth-child(8)').show();
  $('a.gep').toggle();
});
$( '.no-gep' ).live( 'click', function() {
  $('td:nth-child(7)').hide();
  $('td:nth-child(8)').hide();
});
$( '.prereq' ).live( 'click', function() {
  $('td:nth-child(18)').show();
  $('a.prereq').toggle();
});
$( '.no-prereq' ).live( 'click', function() {
  $('td:nth-child(18)').hide();
});

$( '.campus' ).live( 'click', function() {
  $('td:nth-child(10)').show();
  $('a.campus').toggle();
});

$( '.no-campus' ).live( 'click', function() {
  $('td:nth-child(10)').hide();
});

$( '.format' ).live( 'click', function() {
  $('td:nth-child(11)').show();
  $('a.format').toggle();
});

$( '.no-format' ).live( 'click', function() {
  $('td:nth-child(11)').hide();
});

$( '.days' ).live( 'click', function() {
  $('td:nth-child(12)').show();
  $('a.days').toggle();
});

$( '.no-days' ).live( 'click', function() {
  $('td:nth-child(12)').hide();
});


$('a.category').not('a.last').after( ' | ' );
$('a.category-stem').not('a.last').after( ' | ' );

/*Hide Session 1 Class
var session1 = 'Session 1';
$("#table tr td:contains('" + session1  + "')").each(function() {
    if ($(this).text() == session1) {
        $(this).parent().remove();
    }  
});
*/

/*Hide 12 Week Classes
var twelveWeek = '12 Week';
$("#table tr td:contains('" + twelveWeek  + "')").each(function() {
    if ($(this).text() == twelveWeek) {
        $(this).parent().remove();
    }  
});
*/




/* Creates Hash Functionality */
    var hash = window.location.hash.substring(1).replace('%20',' ');
	$( document ).ready(function() {
        $("#search").val(hash);
		$("#search").trigger(e);
		ga('send', 'event',  'search', 'hash', hash);
	});
	
	$( document ).ready(function() {
        if (hash == 'prereq') {
			$('td:nth-child(18)').show();
  			$('a.prereq').toggle();
			$("#search").val('Has Prerequisite');
			$("#search").trigger(e);
	}});
	
	$( document ).ready(function() {
        if (hash == 'gep') {
			$('td:nth-child(7)').show();
			$('td:nth-child(8)').show();
			$('a.gep').toggle();
			$("#search").val('Has GEP');
			$("#search").trigger(e);
	}});
	
	$( document ).ready(function() {
        if (hash == 'evening') {
			$('td:nth-child(13)').show();
			$('td:nth-child(14)').show();
			$('a.time').toggle();
			$("#search").val('Evening');
			$("#search").trigger(e);
	}});
	
		$( document ).ready(function() {
        if (hash == 'online' || hash == 'hybrid') {
			$('td:nth-child(11)').show();
			$('a.format').toggle();
			$("#search").val(hash);
			$("#search").trigger(e);
	}});
	
		$( document ).ready(function() {
        if (hash == 'shadygrove') {
			$('td:nth-child(10)').show();
			$('a.campus').toggle();
			$("#search").val('Shady Grove');
			$("#search").trigger(e);
	}});

/* Creates Special STEM page.*/
	$( document ).ready(function() {
        if (hash == 'STEM') {
			$('td:nth-child(23)').show();
			$('a.sessions').toggle();
			$('.stem').show();
			$("#search").val('*STEM');
			$("#search").trigger(e);
			$(".geps").hide();
			$(".prereq1").hide();
			$(".date-times").hide();
			$(".location").hide();
			$('#head-image').attr('src','images/stem.jpg');
			$('#summer-heading .remove-bottom').css('background-color', '#FFF');
			$('.head-link').attr('href', 'stem');
			$(".page-title:contains('Summer 2017 Courses')").html("Summer STEM 2017 Courses");
			$('.searchbox').hide();
			$('.hide-stem').hide();
	}});
	
$('td:nth-child(21)').each(function() {	
	var subjectForStem = $(this).closest('td').siblings(':nth-child(1)').text();
	if(subjectForStem == 'BIOL' || 
	subjectForStem == 'CHEM' ||
	subjectForStem == 'CMSC' ||
	subjectForStem == 'ENCH' ||
	subjectForStem == 'ENME' ||
	subjectForStem == 'IS' ||
	subjectForStem == 'MATH' ||
	subjectForStem == 'PHYS' ||
	subjectForStem == 'STAT' ||
	subjectForStem == 'EHS' ||
	subjectForStem == 'GES' ||
	subjectForStem == 'SCI'
	){
		$(this).text('*STEM')	
	}
});
	

	/* Fix Class Number Typo & Add Link */
	$( "td[lass='number']" ).each(function() {
		$(this).addClass('number');
		$(this).wrapInner("<a href='' class='course-link'></a>");
	});
	$('#table td:eq(3)').text('Course Title');


	
	/* Creates HighPoint Links*/
	$('.course-link').each(function() {
	var courseURL = $(this).text();
	var title = $(this).closest('td').siblings(':nth-child(3)').text();
	var term = $(this).closest('td').siblings(':nth-child(9)').text();
	var courseFullURL = 'https://highpoint-prd.ps.umbc.edu/app/catalog/classsection/UMBC1/' + term + '/' + courseURL;
	$(this).attr("href", courseFullURL);
	$(this).text(title);
	$(this).attr("target", "_blank");
});

	/* Creates Syllabus Links*/
	$('.syllabus-link').each(function() {
	var courseSubject = $(this).closest('td').siblings(':nth-child(1)').text();
	var courseNumber = $(this).closest('td').siblings(':nth-child(2)').text();
	var courseFull = courseSubject + courseNumber;
	var courseSyllabusURL = 'documents/' + courseFull + '.pdf';
	var regexCourseFull = new RegExp(courseFull);
	$(this).attr("href", courseSyllabusURL);
	$(this).text("Download");
	$(this).attr("target", "_blank");
	if($('.syllabi-list').text().match(regexCourseFull)){
		$(this).show();
	}
	else $(this).hide();
});


/* Add "s"to Week" and "Session" */
$('td:nth-child(5):gt(0)').each(function() {
	if($(this).text().length <= 2){
		$(this).append(' Weeks');
	}
	else {
		$(this).append('s');
	}
});
$('td:nth-child(6):gt(0)').each(function() {
	if($(this).text() == 1 || $(this).text() == 2) {
		$(this).prepend('Session ');
	} 
	if ($(this).text() == ' Winter ') { 
		$(this).append('Session');
	}
	if ($(this).text() == 'W') { 
		$(this).text('12 Week Course');
	}
});



/* Check URL & Hide Courses Accordingly 
var currentSession = window.location.href.toString().split("/")[3];
$('td:nth-child(9):gt(0)').each(function() {
    if ($(this).text().endsWith('6') && currentSession == 'winter') {
        $(this).parent().remove();
	}
	else if ($(this).text().endsWith('0') && currentSession == 'summer') {
		$(this).parent().remove();
    }  
});
*/



/* Make Campus User Friendly */
$('td:nth-child(10)').each(function() {
	if($(this).text() == 'MAIN'){
		$(this).text('Main Campus')
	}
	if($(this).text() == 'SHGR'){
		$(this).text('Shady Grove')
	}
	if($(this).text() == 'OFF'){
		$(this).text('Off Campus')
	}
});

/* Remove Time of Day from Online Classes */
$('td:nth-child(15)').each(function() {	
	if($(this).closest('td').siblings(':nth-child(11)').text() == 'Online'){
		$(this).text('')	
		$(this).closest('td').siblings(':nth-child(13)').text('')
		$(this).closest('td').siblings(':nth-child(14)').text('')	
	}
});

/* Remove Time of Day from 12:00AM Classes */
$('td:nth-child(15)').each(function() {	
	if($(this).closest('td').siblings(':nth-child(13)').text() == '12:00 AM'){
		$(this).text('')	
		$(this).closest('td').siblings(':nth-child(13)').text('')
		$(this).closest('td').siblings(':nth-child(14)').text('')	
	}
});

/* Sends More Details to Google Analytics*/
$('.details').click(function() {
	var detailsText = $(this).text();
	ga('send', 'event',  'search', 'details', detailsText);
});

</script>


</body>
</html>